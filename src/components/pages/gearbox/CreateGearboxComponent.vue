<template>
  <b-card class="m-5" v-if="active">
    <b-card-title class="text-center">Cadastro de caixa de marchas</b-card-title>
    <b-form @submit.prevent="storeGearBox()">
        <b-row>
          <b-col>
            <b-form-group label="Nome" label-for="input-name">
              <b-form-input id="input-name" type="text" v-model="form.inputs.name" placeholder="Digite o nome da caixa de marcha" :class="form.errors.name ? 'has-error-custom' : ''">
              </b-form-input>
        ...
        <b-row class="mt-5">
          ...
          <b-col class="d-flex justify-content-center">
            <b-button type="submit" variant="success" @click.prevent="storeGearBox()" :disabled="form.loading">
...
export default { 
  ...
  methods: {
    ...
    afterSuccessfulStore () {
      this.showSuccessMessage("Conluído! Caixa de marcha cadastrada com sucesso!");
      this.close();
    },
    ...
    afterErrorStore (error) {
      ...
      this.showErrorMessage("Ops! Ocorreu um erro ao cadastrar uma nova caixa de marcha!")
    },
